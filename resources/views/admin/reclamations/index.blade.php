...
@can('reclamation_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.reclamations.create") }}">
                {{ trans('global.add') }} {{ trans('cruds.reclamation.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header grad1">
        {{ trans('cruds.reclamation.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Reclamation">
            <thead class="grad2">
                <tr>
                    <th width="10">
...
$('.card-body').on('change', 'select', function() {
  ...
  let dtButtons = []
@can('reclamation_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.reclamations.massDestroy') }}",
...
$(".datatable-Reclamation").one("preInit.dt", function () {
 $(".dataTables_filter").after(filters);
});
  $('.datatable-Reclamation').DataTable(dtOverrideGlobals);
